...
app.controller('order-ctrl', function ($scope, $http) {
	...
	$scope.confirm = function (id) {
		var url = `${urlOrder}/confirm/${id}`;
		swal({
			title: `Xác nhận đơn hàng ${id}?`,
			text: "Đơn hàng sẽ chuyển sang trạng thái đang giao hàng",
			type: 'warning',
			...
			if (result.value) {
				$http.post(url).then(resp => {
					let index = $scope.orders.findIndex(u => u.id == id);
					$scope.orders[index] = resp.data;
					$scope.order.order = resp.data;
				});
			}
		});
	}
	$scope.success = function (id) {
		var url = `${urlOrder}/success/${id}`;
		swal({
			title: `Đơn hàng ${id} đã giao thành công?`,
			text: "Đơn hàng sẽ chuyển sang trạng thái thành công và tích điểm cho khách hàng",
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
